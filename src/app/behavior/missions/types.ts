import type { TextKey } from "../translations";

export type GameId =
  | 'me1'
  | 'me2'
  | 'me3'
  ;

export type SectionId =
  | 'me1_story'
  | 'me1_citadel'
  | 'me1_squad'
  | 'me1_feros'
  | 'me1_noveria'
  | 'me1_virmire'
  | 'me1_unc'
  | 'me1_bring_down_the_sky'
  | 'me1_collectibles'
  | 'me2_plot'
  | 'me2_dossiers'
  | 'me2_citadel'
  | 'me2_loyalty'
  | 'me2_omega'
  | 'me2_normandy'
  | 'me2_n7'
  | 'me2_illium'
  | 'me2_tuchanka'
  | 'me2_project_firewalker'
  | 'me2_project_overlord'
  | 'me2_lair_of_the_shadow_broker'
  | 'me2_arrival'
  | 'me3_priority'
  | 'me3_priority_the_citadel1'
  | 'me3_priority_palaven'
  | 'me3_priority_surkesh'
  | 'me3_priority_tuchanka'
  | 'me3_priority_the_citadel2'
  | 'me3_priority_geth_dreadnought'
  | 'me3_priority_rannoch'
  | 'me3_priority_thessia'
  ;

export type MissionId =
  | 'me1_eden_prime'
  | 'me1_citadel_expose_saren'
  | 'me1_find_liara'
  | 'me1_noveria_geth_interest'
  | 'me1_noveria_matriarch_benezia'
  | 'me1_feros_geth_attack'
  | 'me1_feros_the_thorian'
  | 'me1_virmire'
  | 'me1_virmire_assault'
  | 'me1_ilos'
  | 'me1_ilos_trench_run'
  | 'me1_final_battle'
  | 'me1_citadel_garrus'
  | 'me1_citadel_wrex'
  | 'me1_citadel_asari_consort'
  | 'me1_citadel_doctor_michel'
  | 'me1_citadel_homecoming'
  | 'me1_citadel_jahleeds_fears'
  | 'me1_citadel_presidium_prophet'
  | 'me1_citadel_reporters_request'
  | 'me1_citadel_ritas_sister'
  | 'me1_citadel_scan_the_keepers'
  | 'me1_citadel_schells_the_gambler'
  | 'me1_citadel_signal_tracking'
  | 'me1_citadel_the_fan'
  | 'me1_citadel_xeltans_complaint'
  | 'me1_citadel_old_friends'
  | 'me1_citadel_old_unhappy_far_off_things'
  | 'me1_citadel_i_remember_me'
  | 'me1_citadel_family_matter'
  | 'me1_citadel_planting_a_bug'
  | 'me1_citadel_snap_inspection'
  | 'me1_citadel_the_fourth_estate'
  | 'me1_citadel_our_own_worst_enemy'
  | 'me1_citadel_negotiators_request'
  | 'me1_garrus_find_dr_saleon'
  | 'me1_tali_and_the_geth'
  | 'me1_wrex_family_armor'
  | 'me1_feros_data_recovery'
  | 'me1_feros_geth_in_the_tunnels'
  | 'me1_feros_power_cells'
  | 'me1_feros_varren_meat'
  | 'me1_feros_water_restoration'
  | 'me1_noveria_espionage'
  | 'me1_noveria_smuggling'
  | 'me1_virmire_assisting_kirrahes_team'
  | 'me1_virmire_wrex_and_the_genophage'
  | 'me1_unc_distress_call'
  | 'me1_unc_asari_diplomacy'
  | 'me1_unc_espionage_probe'
  | 'me1_unc_geth_incursions'
  | 'me1_unc_missing_marines'
  | 'me1_unc_cerberus'
  | 'me1_unc_hades_dogs'
  | 'me1_unc_colony_of_the_dead'
  | 'me1_unc_derelict_freighter'
  | 'me1_unc_exogeni_facility'
  | 'me1_unc_dead_scientists'
  | 'me1_unc_listening_post_alpha'
  | 'me1_unc_listening_post_theta'
  | 'me1_unc_depot_sigma_23'
  | 'me1_unc_hostage'
  | 'me1_unc_hostile_takeover'
  | 'me1_unc_lost_freighter'
  | 'me1_unc_lost_module'
  | 'me1_unc_major_kyle'
  | 'me1_unc_missing_survey_team'
  | 'me1_unc_privateers'
  | 'me1_unc_rogue_vi'
  | 'me1_unc_besieged_base'
  | 'me1_unc_the_negotiation'
  | 'me1_x57_bring_down_the_sky'
  | 'me1_x57_avoid_the_blasting_caps'
  | 'me1_x57_missing_engineers'
  | 'me1_unc_prothean_data_discs'
  | 'me1_unc_turian_insignias'
  | 'me1_unc_valuable_minerals'
  | 'me1_unc_locate_signs_of_battle'
  | 'me1_unc_asari_writings'
  | 'me2_prologue_save_joker'
  | 'me2_prologue_awakening'
  | 'me2_freedoms_progress'
  | 'me2_citadel_anderson'
  | 'me2_recruit_archangel'
  | 'me2_recruit_professor'
  | 'me2_recruit_warlord'
  | 'me2_recruit_convict'
  | 'me2_horizon'
  | 'me2_recruit_assassin'
  | 'me2_recruit_justicar'
  | 'me2_recruit_tali'
  | 'me2_collector_ship'
  | 'me2_iff'
  | 'me2_await_iff'
  | 'me2_use_omega4_relay'
  | 'me2_suicide_mission_infiltration'
  | 'me2_suicide_mission_the_long_walk'
  | 'me2_suicide_mission_final_battle'
  | 'me2_citadel_crime_in_progress'
  | 'me2_citadel_found_forged_id'
  | 'me2_citadel_krogan_sushi'
  | 'me2_citadel_captain_bailey'
  | 'me2_citadel_the_council'
  | 'me2_dossier_the_master_thief'
  | 'me2_dossier_the_veteran'
  | 'me2_loyalty_kasumi'
  | 'me2_loyalty_zaeed'
  | 'me2_loyalty_garrus'
  | 'me2_loyalty_grunt'
  | 'me2_loyalty_jack'
  | 'me2_loyalty_jacob'
  | 'me2_loyalty_miranda'
  | 'me2_loyalty_mordin'
  | 'me2_loyalty_samara'
  | 'me2_loyalty_tali'
  | 'me2_loyalty_thane'
  | 'me2_loyalty_legion'
  | 'me2_omega_aria_tloak'
  | 'me2_omega_archangel_datapad_recovered'
  | 'me2_omega_batarian_bartender'
  | 'me2_omega_packages_for_ish'
  | 'me2_omega_struggling_quarian'
  | 'me2_omega_the_patriarch'
  | 'me2_omega_the_professor_missing_assistant'
  | 'me2_normandy_fba_couplings'
  | 'me2_normandy_serrice_ice_brandy'
  | 'me2_normandy_special_ingredients'
  | 'me2_normandy_crash_site'
  | 'me2_n7_wrecked_merchant_freighter'
  | 'me2_n7_abandoned_research_station'
  | 'me2_n7_hahne_kedar_facility'
  | 'me2_n7_msv_estevanico'
  | 'me2_n7_eclipse_smuggling_depot'
  | 'me2_n7_lost_operative'
  | 'me2_n7_abandoned_mine'
  | 'me2_n7_anomalous_weather_detected'
  | 'me2_n7_blood_pack_base'
  | 'me2_n7_blood_pack_communications_relay'
  | 'me2_n7_quarian_crash_site'
  | 'me2_n7_imminent_ship_crash'
  | 'me2_n7_archeological_dig_site'
  | 'me2_n7_msv_strontium_mule'
  | 'me2_n7_blue_suns_base'
  | 'me2_n7_javelin_missiles_launched'
  | 'me2_n7_captured_mining_facility'
  | 'me2_n7_endangered_research_station'
  | 'me2_n7_mining_the_canyon'
  | 'me2_illium_liara_system_hacking'
  | 'me2_illium_liara_the_observer'
  | 'me2_illium_a_troublemaker'
  | 'me2_illium_blue_rose_of_illium'
  | 'me2_illium_gianna_parasini'
  | 'me2_illium_indentured_service'
  | 'me2_illium_medical_scans'
  | 'me2_illium_the_assassin_salarian_family_data'
  | 'me2_illium_the_justicar_smuggling_evidence'
  | 'me2_illium_the_justicar_stolen_goods_found'
  | 'me2_illium_the_prodigal_lost_locket_found'
  | 'me2_tuchanka_combustion_manifold'
  | 'me2_tuchanka_killing_pyjaks'
  | 'me2_tuchanka_old_blood_missing_scout'
  | 'me2_tuchanka_urdnot_wrex'
  | 'me2_project_firewalker_rosalie_lost'
  | 'me2_project_firewalker_geth_activity'
  | 'me2_project_firewalker_survey_sites_located'
  | 'me2_project_firewalker_volcano_station'
  | 'me2_project_firewalker_prothean_site'
  | 'me2_overlord'
  | 'me2_overlord_atlas_station'
  | 'me2_overlord_prometheus_station'
  | 'me2_overlord_vulcan_station'
  | 'me2_lair_of_the_shadow_broker'
  | 'me2_arrival'
  | 'me3_priority_prologue'
  | 'me3_priority_mars'
  | 'me3_priority_citadel1'
  | 'me3_priority_palaven'
  | 'me3_priority_surkesh'
  | 'me3_priority_tuchanka'
  | 'me3_priority_citadel2'
  | 'me3_priority_perseus_veil'
  | 'me3_priority_geth_dreadnought'
  | 'me3_priority_rannoch'
  | 'me3_priority_citadel3'
  | 'me3_priority_thessia'
  | 'me3_priority_horizon'
  | 'me3_priority_cerberus_hq'
  | 'me3_priority_earth'
  | 'me3_n7_cerberus_lab'
  | 'me3_priority_eden_prime'
  | 'me3_eden_prime_resistance_movement'
  | 'me3_shrike_abyssal_prothean_obelisk'
  | 'me3_apien_crest_banner_of_the_first_regiment'
  | 'me3_aria_blood_pack'
  | 'me3_aria_blue_suns'
  | 'me3_aria_eclipse'
  | 'me3_benning_evidence'
  | 'me3_citadel_alien_medi_gel_formula'
  | 'me3_citadel_aria_tloak'
  | 'me3_citadel_barla_von'
  | 'me3_citadel_biotic_amp_interfaces'
  | 'me3_citadel_dr_bryson'
  | 'me3_citadel_gx12_thermal_pipe'
  | 'me3_citadel_hanar_diplomat'
  | 'me3_citadel_heating_unit_stabilizers'
  | 'me3_citadel_improved_power_grid'
  | 'me3_grissom_academy_emergency_evacuation'
  | 'me3_irune_book_of_plenix'
  | 'me3_ismar_frontier_prototype_components'
  | 'me3_kites_nest_pillars_of_strength'
  | 'me3_attican_traverse_krogan_team'
  | 'me3_citadel_krogan_dying_message'
  | 'me3_n7_cerberus_abductions'
  | 'me3_n7_cerberus_attack'
  | 'me3_tuchanka_turian_platoon'
  | 'me3_citadel_cerberus_automated_turret_schematics'
  | 'me3_tuchanka_bomb'
  | 'me3_n7_cerberus_fighter_base'
  | 'me3_arrae_ex_cerberus_scientists'
  | 'me3_athena_nebula_hesperia_period_statue'
  | 'me3_citadel_batarian_codes'
  | 'me3_citadel_cerberus_ciphers'
  | 'me3_citadel_cerberus_retribution'
  | 'me3_citadel_cerberus_turian_poison'
  | 'me3_citadel_chemical_treatment'
  | 'me3_citadel_inspirational_stories'
  | 'me3_citadel_kakliosaur_fossil'
  | 'me3_citadel_medical_supplies'
  | 'me3_citadel_medi_gel_sabotage'
  | 'me3_citadel_reaper_code_fragments'
  | 'me3_citadel_shore_leave'
  | 'me3_citadel_target_jamming_technology'
  | 'me3_citadel_volus_ambassador'
  | 'me3_citadel_wounded_batarian'
  | 'me3_dekuuna_code_of_the_ancients'
  | 'me3_hades_nexus_obelisk_of_karza'
  | 'me3_hades_nexus_prothean_sphere'
  | 'me3_kallini_ardat_yakshi_monastery'
  | 'me3_citadel_asari_widow'
  | 'me3_nimbus_cluster_library_of_asha'
  | 'me3_silean_nebula_rings_of_alune'
  | 'me3_valhallan_threshold_prothean_data_drives'
  | 'me3_n7_fuel_reactors'
  | 'me3_rannoch_admiral_koris'
  | 'me3_rannoch_geth_fighter_squadrons'
  | 'me3_dekuuna_elcor_extraction'
  | 'me3_n7_communication_hub'
  ;

export type Mission = {
  id: MissionId;
  prerequisites: MissionId[];
  additionalInfo?: TextKey;
  wikiUrl: string;
};

export type Section = {
  id: SectionId;
  missions: Mission[];
};

export type Game = {
  id: GameId;
  sections: Section[];
};

export type GameData = {
  games: Game[];
};
